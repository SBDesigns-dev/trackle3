...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-overground") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-overground", JSON.stringify(places));
}
...
App.prototype.resetGame = function () {
  // Clear local storage
  window.localStorage.removeItem("places-overground");
